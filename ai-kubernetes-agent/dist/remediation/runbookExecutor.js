...
export class RunbookExecutor {
    ...
    async execute(incident, actionId, approved) {
        const action = incident.recommended_actions.find((item) => item.id === actionId);
        if (!action)
            throw new Error(`Action not found: ${actionId}`);
        if (action.requires_approval && !approved)
            throw new Error("Approval is required before this runbook can execute.");
        if (this.locks.has(incident.incident_id))
            throw new Error("Another remediation is already running for this incident.");
        ...
        try {
            ...
            return {
                runbook: action.runbook,
                changedCluster: false,
                result: "Mutation runbooks are intentionally stubbed in Phase 1. Enable only after approvals, locking, verification, and RBAC are tested."
            };
        }
        finally {
