...
export type Role = "viewer" | "operator" | "approver" | "administrator";
...
declare module "express-serve-static-core" {
  interface Request {
    actor?: { role: Role; subject: string };
  }
}
...
export function requireRole(role: Role) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const headerRole = (req.header("x-agent-role") ?? "viewer") as Role;
...
export function verifyAlertmanagerSecret(req: Request, res: Response, next: NextFunction) {
  if (!config.ALERTMANAGER_SHARED_SECRET) return next();
